...
/****** Object:  View [dbo].[vw_ServicesRevenueForecastReport_run_3]    Script Date: 10/23/2014 10:34:51 ******/
SET ANSI_NULLS ON
GO
...
ALTER VIEW [dbo].[vw_ServicesRevenueForecastReport_run_3] 
AS 
SELECT ProjE.new_title AS [Title]
	...
	, NULL  AS [Opp Close Probability]
	, CASE Proj.StateCode WHEN 0 THEN 'Active' WHEN 1 THEN 'Inactive' ELSE 'Undefined' END AS [State] 
	, 'Proj' AS [Record Type]
...
SELECT Opp.Name AS [Title]
	...
	, OppE.new_closeprobability AS [Opp Close Probability]
	, CASE Opp.StateCode WHEN 0 THEN 'Open' WHEN 1 THEN 'Won' WHEN 2 THEN 'Lost' ELSE 'Undefined' END AS [State] 
	, 'Opp' AS [Record Type]
