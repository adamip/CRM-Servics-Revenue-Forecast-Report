...
SELECT ProjE.new_title AS [Title]
	, ProjE.new_StartDate AS [Project Start Date]
	, ProjE.new_EstCompletion AS [Project Estimated Completion]
	...
	, ProjE.new_customerPO AS [Customer PO], ProjE.cust_CustomerPOreceivedDate AS [Customer PO Received Date]
	, CASE ProjE.new_COSreceived WHEN 1 THEN 'True' WHEN 0 THEN 'False' ELSE '' END AS [COS Received]
	, ProjE.cust_COSreceivedDate AS [COS Received Date]
...
SELECT Opp.Name AS [Title], Opp.EstimatedCloseDate AS [Project Start Date]
	...
	, OppE.new_POnumber AS [Customer PO], NULL AS [Customer PO Received Date]
